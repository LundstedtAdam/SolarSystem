...
//
//   Hyperdrive (tiers 1-3)  — which distance band you can even fly to.
...
export type UpgradeKind = 'hyperdrive' | 'scanner' | 'shielding' | 'cargo';
export const UPGRADE_KINDS: UpgradeKind[] = ['hyperdrive', 'scanner', 'shielding', 'cargo'];
...
// --- Hyperdrive distance bands -----------------------------------------------

export type DistanceBand = 'home' | 'near' | 'inner' | 'outer';

/** Which hyperdrive tier is required to reach each band. 'home' needs none —
...
export const DEFAULT_UPGRADES: ShipUpgrades = { hyperdrive: 0, scanner: 0, shielding: 0, cargo: 0 };

export interface DescentGate {
  ok: boolean;
  /** i18n-free reason code the HUD renders; null when ok. */
  reason: 'hyperdrive' | 'shielding' | null;
  /** For a hyperdrive block: the tier that's actually needed. */
  neededTier?: number;
}
...
export function canDescend(name: string, upgrades: ShipUpgrades): DescentGate {
  ...
  if (upgrades.hyperdrive < needed) return { ok: false, reason: 'hyperdrive', neededTier: needed };
